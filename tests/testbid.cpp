...
struct TestBid : public ::testing::Test
{
	...
	{
		return QTacitGame::GameMapBuilder::fromJson(R"STR({ "states" : [ ")STR" + state1name
													+ R"STR(", ")STR" + state2name + R"STR(", ")STR"
													+ state3name + R"STR(" ] })STR");
	}

	TestBid()
		: map(createMap())
		, bid(map)
		, state1(map.indexOf(state1name))
		, state2(map.indexOf(state2name))
		, state3(map.indexOf(state3name))
	{
	}
};
